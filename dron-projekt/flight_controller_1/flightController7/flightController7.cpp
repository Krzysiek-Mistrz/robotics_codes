...
int ThrottleIdle = 1180;    //(18% power [from 1000, because: 1000:2000])
int ThrottleCutOff = 1500;  //because of esc specification
int MinMicroseconds = 1000; 
int MaxMicroseconds = 2000;
float GainConstant = 5; //experimental constant increase constant -> more powerful PID
bool WasThereFlight = 0;
unsigned long Time;
int MaxFlyable = 1800;  //80% power 20% for PID regulation
int MinFlyable = 1650;  //65% power to fly down without damage
//gy521 vars
...
void update(int ThrottleValue) {
    ...
    pidEquation(ErrorRateYaw, PRateYaw,
...
void fly(void) {
    if (WasThereFlight == 0) {
        ...
        motor1.writeMicroseconds(MotorPWM);
        motor2.writeMicroseconds(MotorPWM);
        motor3.writeMicroseconds(MotorPWM);
        motor4.writeMicroseconds(MotorPWM);
        //waitin for the motor initial setup
        delay(250);

        //increasing throttle
        for (int i = MotorPWM; i < MaxFlyable; i++) {
            update(i);
        }

        //fly up until less than 5s
        Time = millis();
        ...
        //decreasing throttle
        for (int i = MaxFlyable; i > MinFlyable; i--) {
            update(i);
        }

        //falling
        Time = millis();
        while (millis() - Time < 20000UL) {
            update(MinFlyable);
        }

        //slowin motors to no rotation pwm value
        for (int i = MinFlyable; i > MotorPWM; i--) {
            update(i);
        }
