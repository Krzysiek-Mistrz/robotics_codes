...
//pin<->motor: 3=UPPER_RIGHT=motor1, 9=UPPER_LEFT=motor4, 10=DOWN_LEFT=motor3, 11=DOWN_RIGHT=motor2
int MotorPWM = 1500;
unsigned long Time;
...
void update(int ThrottleValue) {
    ...
    //settings pwm value to motors
    analogWrite(ESCPIN11, MotorInput1);
    analogWrite(ESCPIN3, MotorInput2);
    analogWrite(ESCPIN9, MotorInput3);
    ...
    while (micros() - LoopTimer < 4000);    //1s = 1000ms = 1000000us -> waitin 4 microseconds
    LoopTimer=micros();
}
